...
from app.services.portal_crawler import crawl_portal
...
def _run(job_id: int, list_url: str, limit: int) -> None:
    ...
    with Session(engine) as db:
        ...
        if results:
            ...
            session_row = RecordingSession(
                ...
                status="COMPLETED",
                kind="portal",
                source_label=f"일괄 수집 · 서비스 {len(results)}개",
            )
            db.add(session_row)
            db.commit()
...
def start(project_id: int, list_url: str, limit: int) -> int:
    ...
    thread = threading.Thread(target=_run, args=(job_id, list_url, limit), daemon=True)
    thread.start()
    return job_id
